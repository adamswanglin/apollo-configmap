...
var _ = Describe("ApolloConfigServer Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind ApolloConfigServer")
			err := k8sClient.Get(ctx, typeNamespacedName, apolloconfigserver)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ApolloConfigServer")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &ApolloConfigServerReconciler{
				Client: k8sClient,
